...
Copyright (C) 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2022, 2023  Konrad Bernloehr
...
void user_set_pixel_stats (int on)
{
   ...
   if ( c < 0 || c > MAX_TEL_TYPES )
      return;
   up[c].i.pixstat = on;
printf("Setting pixel trigger statistics for telescope type %d to %d\n", c, on);
   if ( c == 0 )
   {
      int i;
      for ( i=1; i<=MAX_TEL_TYPES; i++ )
...
static void book_hist_for_type ( UNUSED_PAR2(AllHessData *,hsdata), int tel_type)
{
   double xylow[2], xyhigh[2];
   int nbins[2];
   ...
   xyhigh[0] = 100.;
   nbins[0]  = 1000;  
   book_histogram(ht+21001,
      "Pixel calibrated amplitude (p.e.), all pixels",
      "D", 1, xylow, xyhigh, nbins);
   book_histogram(ht+21002,
      "Pixel calibrated amplitude (p.e.), triggered pixels",
      "D", 1, xylow, xyhigh, nbins);
   book_histogram(ht+21011,
      "Trigger group sum of pixel calibrated amplitudes (p.e.), all groups",
      "D", 1, xylow, xyhigh, nbins);
   book_histogram(ht+21012,
      "Trigger group sum of pixel calibrated amplitudes (p.e.), triggered groups",
      "D", 1, xylow, xyhigh, nbins);
...
static void user_mc_shower_fill ( UNUSED_PAR2(AllHessData *,hsdata) /* currently unused */)
{
...
static void user_event_fill (AllHessData *hsdata, int stage)
{
   ...
   else if ( E_true == 0. ) /** == 0. may happen for calibration events */
   {
      if ( verbosity > 0 )
         printf("Skip user_event_fill for calibration-type event.\n");
      return;
   }
   ...
   /* Example code: */
      ...
      if ( n_img3 >= 2 && w_hmax > 0. )
      {
         ...
         {
            AtmProf *aprof = get_common_atmprof();
            int rc = 0;
            if ( aprof->n_alt > 1 ) /* Prefer profile from table in input data file */
               rc = init_atmprof_s(aprof);
            else if ( hsdata->mc_run_header.atmosphere > 0 && hsdata->mc_run_header.atmosphere < 99 )
            {
               /* If there is no table in the data, we look for a file matching the atmosphere ID. */
               rc = init_atmprof(hsdata->mc_run_header.atmosphere);
               if ( rc != 0 && aprof->have_lay5_param ) /* In case of failure we might have a backup */
                  rc = init_atmprof_s(aprof);           /* in the 5-layer parameters. */
            }
            else if ( aprof->have_lay5_param )
               rc = init_atmprof_s(aprof);
            have_atm_set = hsdata->mc_run_header.atmosphere;
            if ( have_atm_set == 0 && rc == 0 ) /* We made a table from 5-layer parameters. */
               have_atm_set = hsdata->mc_run_header.atmosphere = 100;
         }
...
static void user_done ( UNUSED_PAR2(AllHessData *,hsdata) )
{
}
...
static char *prog_path ()
{
   ...
   char cmdl_fname[1024], program[PATH_MAX+4], path_elem[PATH_MAX];
   pid_t i = getpid();
   int n;
   size_t nu;
   FILE *f;
   ...
   {
      strncpy(bf,".",2); /* Probably not Linux. Need another trick. */
      return bf;
   }
   for (nu=0; nu+1<bs; nu++)
      if ( (bf[nu] = fgetc(f)) == '\0' )
   ...
   {
      strncpy(bf,".",2); /* Perhaps the program itself destroyed PATH. */
      return bf;
   }
...
static void user_finish (AllHessData *hsdata)
{
   ...
   if ( hsdata != NULL && hsdata->mc_shower.primary_id == 0 )
   {
      if ( auto_lookup )
      {
         ...
         snprintf(cmd,sizeof(cmd)-1,"%s/gen_lookup",bsdir);
         if ( access(cmd,X_OK) == 0 )
         {
            snprintf(cmd,sizeof(cmd)-1,"%s/gen_lookup %s %s", bsdir, hist_fname, lookup_fname);
         }
         else if ( strcmp(bsdir,".") == 0 && hessroot != NULL )
         {
            snprintf(cmd,sizeof(cmd)-1,"%s/bin/gen_lookup %s %s", hessroot, hist_fname, lookup_fname);
         }
         else
            snprintf(cmd,sizeof(cmd)-1,"gen_lookup %s %s", hist_fname, lookup_fname);
 ...
 * @param stage Only relevent for the IO_TYPE_SIMTEL_EVENT item type
