/* ============================================================================
   ...
   Copyright (C) 1993, 1997, 2001, 2002, 2003, 2006, 2007, 2008, 2009, 2010, 2013, 2012, 2013, 2016, 2018, 2021  Konrad Bernloehr
...
int write_all_histograms (const char *fname)
{
   ...
   const char *s;
   long buflen = 8000000L;
   int rc;
   
   if ( (s = getenv("HDATA_IO_BUFFER")) != NULL )
   {
      long ble = io_buffer_size_spec(s);
      if ( ble >= 1000000L )
         buflen = ble;
   }
   ...
   if ( (iobuf = allocate_io_buffer(buflen)) == NULL )
      return -1;
   if ( iobuf->max_length < 800000000L )
      iobuf->max_length = 800000000L;
...
*/
   ...
   if ( (iobuf = allocate_io_buffer(buflen)) == (IO_BUFFER *) NULL )
   {
