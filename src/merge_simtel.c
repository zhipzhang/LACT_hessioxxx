...
Copyright (C) 2013, 2014, 2015, 2016, 2017, 2018, 2021, 2022, 2023  Konrad Bernloehr
...
/* ----------------------------------------------------------------------- */
/**
 *  @short Check if previously delayed writing of output should be done now.
 *
 *  @param item_header The item header descriptor.
 *  @param ifile Not used here since there is only one output file.
 *  @param hsdata_out The struct where the merged data gets collected.
 *  @param iobuf_out The output buffer descriptor.
 */
  ...
  (int) item_header->type, item_header->ident, ifile);
   ...
   switch ( (int) item_header->type )
   {
      ...
      case IO_TYPE_SIMTEL_RUNHEADER: /* 2000 */
         ...
         {
            set_tel_idx_ref(0);
            write_simtel_runheader(iobuf_out,&hsdata_out->run_header);
         }
         break;
...
#ifdef DEBUG_MERGE
printf("Meta-parameters written\n");
#endif
         clear_metaparam(mpl);
         free(mpl);
         break;
...
static void syntax (const char *program)
{
   printf("A program for merging events from separate telescope simulations\n"
          "of the same showers.\n\n");
...
int main (int argc, char **argv)
{
   ...
   if ( iobuf2->max_length < 400000000L )
      iobuf2->max_length = 400000000L;
   if ( iobuf3->max_length < iobuf1->max_length+iobuf2->max_length )
      iobuf3->max_length = iobuf1->max_length+iobuf2->max_length;
...
printf("Type1 = %d, ID = %ld\n", this_type1, item_header1.ident);
         ...
         switch ( this_type1 )
         {
            ...
            case 2010: /* IO_TYPE_SIMTEL_EVENT */
               ...
               if ( this_type2 == 70 || this_type2 == 1212 || this_type2 == 75 ||
...
printf("Type2 = %d, ID = %ld\n", this_type2, item_header2.ident);
         ...
         {
            case 70:   /* Immediate output without other actions */
            case 2000: /* IO_TYPE_SIMTEL_RUNHEADER: Merging with immediate output (must have merged run header of file 1) */
            case 2001: /* IO_TYPE_SIMTEL_MCRUNHEADER: Merging without immediate output (must have MC merged run header of file 1) */
            ...
            case 2023: /* IO_TYPE_SIMTEL_LASCAL */
               if ( this_type1 == 70 || this_type1 == 2000 || 
                    this_type1 == 2001 || this_type1 == 1212 || this_type1 == 75 ||
            ...
            case 2010: /* IO_TYPE_SIMTEL_EVENT */
               ...
               if ( this_type1 == 70 || this_type1 == 1212 || this_type1 == 75 ||
