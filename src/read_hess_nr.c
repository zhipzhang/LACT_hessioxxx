...
Copyright (C) 2008, 2009, 2010, 2011, 2018, 2022, 2023  Konrad Bernloehr
 ...
 *
@verbatim
Syntax: read_simtel_nr [ options ] [ - | input_fname ... ]
Options:
...
/** @defgroup read_simtel_nr_c The read_simtel_nr program */
/** @{ */
...
int main (int argc, char **argv)
{
   ...
   if ( iobuf->max_length < 100000000L )
    ...
    for (;;) /* Loop over all data in the input file */
    {
      ...
      switch ( (int) item_header.type )
      {
         ...
         case IO_TYPE_SIMTEL_RUNHEADER:
            ...
            if ( verbose || rc != 0 )
               printf("read_simtel_runheader(), rc = %d\n",rc);
         ...
         case IO_TYPE_SIMTEL_LASCAL:
            ...
            if ( verbose || rc != 0 )
               printf("read_simtel_laser_calib(), rc = %d\n",rc);
            if ( showdata )
