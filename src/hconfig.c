/* ============================================================================
   ...
   Copyright (C) 2001, 2002, 2003, 2004, 2006, 2008, 2010, 2011, 2013, 2014, 2015, 2016, 2018, 2019, 2020, 2021, 2022, 2023  Konrad Bernloehr
...
static int set_config_values (CONFIG_ITEM *item, int first,
   int last, char *text);
static void display_config_initial (CONFIG_ITEM *item);
...
static int bool_config_val (const char *name, const char *text, const char *lbound,
   const char *ubound, int strict, bool *bval);
...
static int f_initlist_config (const char *name, CONFIG_VALUES *val);
static int f_typelist_config (const char *name, CONFIG_VALUES *val);
...
static void config_destructor ( UNUSED_PAR2(void *,whatever)  /* unused */ )
{
    ...
    "Invalid character 0x%02x in configuration item '%s' (section '%s').",
         ...
         if ( block->section != NULL && 
           ...
           if ( strcasecmp(item->name,"all") == 0 ||
                ...
                strcasecmp(item->name,"initlist") == 0 ||
                strcasecmp(item->name,"limits") == 0 ||
                strcasecmp(item->name,"typelist") == 0 ||
         ...
         else if ( abbrev(item->type,"UCharacter") )
         {
            ...
            item->internal.elem_size = sizeof(unsigned char);
            item->type = "UChar";
         }
         ...
         else if ( abbrev(item->type,"Bool") )
         {
            item->internal.itype = 35;
            item->internal.elem_size = sizeof(bool);
            item->type = "Bool";
         }
         else if ( abbrev(item->type,"IBool") )
         {
            item->internal.itype = 36;
            item->internal.elem_size = sizeof(int);
            item->type = "IBool";
         }
         else if ( abbrev(item->type,"UBool") )
         {
            item->internal.itype = 37;
            item->internal.elem_size = sizeof(unsigned int);
            item->type = "IBool";
         }
         else if ( atoi(item->type) > 0 ) /* Why was that atof()? */
         {
            item->internal.itype = -atoi(item->type); /* Same here */
...
/* ------------------------ first_config_item --------------------- */
/**
 *  Get the first known config item.
 *
 *  This may be either the first item globally or the first in a given section.
 *
 *  @param sname Name of section in which to search, or NULL for all sections.
 *
 *  @return Pointer to first configuration item found or NULL.
*/

CONFIG_ITEM *first_config_item (const char *sname)
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no;
   ...
   section[0] = '\0';
   if ( sname != NULL )
      strncpy(section,sname,sizeof(section)-1);
...
/* ------------------------ next_config_item --------------------- */
/**
 *  Get the next known config item. Allows for application-level 
 *  iteration of configuration items without knowing about the internal
 *  organisation.
 *
 *  This may be either the next item globally or the next in a given section.
 *  Note that the iteration is not very efficient since we start from
 *  the beginning each time, the only context being the item name found
 *  in the previous round. Thus searching until we find that one again
 *  and then return the next one.
 *
 *  @param sname Name of section in which to search, or NULL for all sections.
 *  @param cname Name of configuration item resulting from last iteration.
 *               A NULL value here results in a first_config_item() call.
 *
 *  @return Pointer to the next configuration item found or NULL.
*/

CONFIG_ITEM *next_config_item (const char *sname, const char *cname)
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no;
   ...
   if ( cname == NULL )
      return first_config_item(sname);
         ...
         block=block->next )
   {
      ...
      for (item_no=0; itemlist[item_no].name != (char *) NULL; item_no++)
      {
         ...
         if ( found_prev )
            return(item);
         else if ( abbrev(cname,item->name) )
            found_prev = 1;
      }
   }
...
static int do_config (CONFIG_ITEM *item, CONST char *line)
{
        ...
        (item->flags & CFG_NOT_INITIAL) )
   {
      char message[1024];
      snprintf(message,sizeof(message),
         "Modification of configuration item '%s' rejected after initial values.",
         item->name);
      Warning(message);
      return -1;
   }
   ...
   char *text)
{
   ...
   if ( item->internal.itype < 10 )
      ...
      rc = hex_config_val(item->name,text,item->lbound,
         item->ubound,strict,&uval);
   else if ( item->internal.itype == 35 || item->internal.itype == 36 || 
             item->internal.itype == 37 )
      rc = bool_config_val(item->name,text,item->lbound,
         item->ubound,strict,&bval);
   else
   ...
   switch ( item->internal.itype )
   {
      ...
      case 35:
         for (inum=first; inum<=last; inum++)
            data.bdata[inum] = bval;
         break;
      case 36:
         for (inum=first; inum<=last; inum++)
            data.idata[inum] = (int) bval;
         break;
      case 37:
         for (inum=first; inum<=last; inum++)
            data.uidata[inum] = (unsigned int) bval;
         break;
      default:
...
static int f_lock_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */ )
{
...
static int f_unlock_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */ )
{
...
static int f_limit_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */ )
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no;
 
   static int limitlist_all_done = 0;
   
   char *li_style = getenv("HCONFIG_LIST_STYLE");
   char *li_prefix = getenv("HCONFIG_LIST_PREFIX");
   char *li_prefix2 = getenv("HCONFIG_LIMITS_PREFIX");
   char *li_prefix3 = getenv("HCONFIG_SECTION_PREFIX");
   if ( li_prefix3 != NULL && (li_prefix3[0] == '\0' || strcmp(li_prefix3,"0") == 0) )
      li_prefix3 = NULL;
   int ltx_style = 0;
   if ( li_style != NULL )
   {
   ...
   if ( limitlist_all_done ) /* Once is enough */
      return 0;
         ...
         block=block->next )
   {
      ...
      for (item_no=0; itemlist[item_no].name != (char *) NULL; item_no++)
      {
         ...
         if ( name[0] == '\0' || strcmp(name,"*") == 0 || strcmp(name,"all") == 0 )
            ...
            show_it = 1;
         else if ( strcmp(name,"no-internal") == 0 && strcmp(item->internal.values.section,"_internal_") != 0 )
            show_it = 1;
         if ( ! show_it )
            continue;
         int has_lbound = 0, has_ubound = 0;
         if ( item->lbound != (char *) NULL )
            if ( item->lbound[0] != '\0' )
               has_lbound = 1;
         if ( item->ubound != (char *) NULL )
            if ( item->ubound[0] != '\0' )
               has_ubound = 1;
         if ( !has_lbound && !has_ubound )
            continue;
#if 0
         printf("%s\t%s\t%s\n", item->name,
         ...
         { 
            Output(li_prefix);
            if ( li_prefix2 != NULL && strcmp(li_prefix2,"") != 0 )
               Output(li_prefix2);
            if ( ltx_style )
               Output(" & ");
            else
   ...
   if ( name[0] == '\0' || strcmp(name,"*") == 0 || strcmp(name,"all") == 0 || strcmp(name,"no-internal") == 0 )
      limitlist_all_done = 1;
...
static int f_status_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   return lock_unlock_status(name,-1);
}
...
static int f_list_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no;
   ...
   char *li_style = getenv("HCONFIG_LIST_STYLE");
   char *li_prefix = getenv("HCONFIG_LIST_PREFIX");
   char *li_prefix2 = getenv("HCONFIG_PREFIX_NUM");
   char *li_prefix3 = getenv("HCONFIG_SECTION_PREFIX");
   if ( li_prefix3 != NULL && (li_prefix3[0] == '\0' || strcmp(li_prefix3,"0") == 0) )
      li_prefix3 = NULL;
   int ltx_style = 0, tsv_style = 0;
   if ( li_style != NULL )
   {
      if ( strcmp(li_style,"latex") == 0 )
         ltx_style = 1;
      else if ( strcmp(li_style,"tsv") == 0 || strcmp(li_style,"tsv+") == 0  )
         tsv_style = 1;
   }
         ...
         block=block->next )
   {
      ...
      for (item_no=0; itemlist[item_no].name != (char *) NULL; item_no++)
      {
         ...
         if ( name[0] == '\0' || strcmp(name,"*") == 0 || strcmp(name,"all") == 0 )
            ...
            show_it = 1;
         /* Function parameters have no area for assigned values.
            Only try to list them when asked explicitly. */
         if ( strcmp(name,item->name) != 0 && item->internal.itype == 50 )
            show_it = 0;
         /* Totally immutable parameters should not show up for reconfiguration */
         if ( (item->flags & CFG_REJECT_MODIFICATION) )
            show_it = 0;
         if ( ! show_it )
            continue;
...
static int f_initlist_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no;
   static int initlist_all_done = 0;
   
   char *li_style = getenv("HCONFIG_LIST_STYLE");
   char *li_prefix = getenv("HCONFIG_LIST_PREFIX");
   char *li_prefix2 = getenv("HCONFIG_INITLIST_PREFIX");
   char *li_prefix3 = getenv("HCONFIG_SECTION_PREFIX");
   if ( li_prefix3 != NULL && (li_prefix3[0] == '\0' || strcmp(li_prefix3,"0") == 0) )
      li_prefix3 = NULL;
   int ltx_style = 0, tsv_style = 0;
   ...
   if ( initlist_all_done ) /* Once is enough */
      return 0;
...
static int f_typelist_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no;
   static int typelist_all_done = 0;
   char oline[10000], pfx3[100];
   
   char *li_style = getenv("HCONFIG_LIST_STYLE");
   char *li_prefix = getenv("HCONFIG_LIST_PREFIX");
   char *li_prefix2 = getenv("HCONFIG_TYPELIST_PREFIX");
   char *li_prefix3 = getenv("HCONFIG_SECTION_PREFIX");
   if ( li_prefix3 != NULL && (li_prefix3[0] == '\0' || strcmp(li_prefix3,"0") == 0) )
      li_prefix3 = NULL;
   int ltx_style = 0, tsv_style = 0;
   const char *del1 = " = ", *del2 = " ", *del3 = "\n";
   const char *pfx = "", *pfx2 = "", *delpfx = "";
   const char *tp = "?";
   if ( li_style != NULL )
   {
      if ( strcmp(li_style,"latex") == 0 )
   ...
   if ( typelist_all_done ) /* Once is enough */
      return 0;
         ...
         block=block->next )
   {
      if ( (itemlist=block->items) == (CONFIG_ITEM *) NULL )
         break;
      if ( li_prefix3 != NULL && strlen(li_prefix3) + 6 < sizeof(pfx3) )
      {
         snprintf(pfx3, sizeof(pfx3)-1, "%s[%s]", del2, block->section);
      }
      else
         pfx3[0] = '\0';
      for (item_no=0; itemlist[item_no].name != (char *) NULL; item_no++)
      {
         ...
         if ( item->type != (char *) NULL )
            ...
            tp = "?";
         snprintf(oline,sizeof(oline)/2,"%s%s%s%s%s%s%s%s%d%s%s%s",
            pfx,pfx2,pfx3,delpfx, item->name, del1,
            tp, del2, item->size, del2,
            (item->internal.bound&0x04)?"L":((item->internal.bound&0x01)?"l":""),
            (item->internal.bound&0x08)?"U":((item->internal.bound&0x02)?"u":""));
         if ( item->internal.locked || (item->flags&CFG_REJECT_MODIFICATION) )
            strcat(oline,"R");
         if ( (item->flags&CFG_HARD_BOUND) )
            strcat(oline,"H");
         if ( (item->flags&CFG_STRICT_BOUND) )
...
static int f_get_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   return 0;
}
...
static int f_echo (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   fprintf(stdout,"%s\n", name);
   return 0;
}
...
static int f_warning (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   fprintf(stderr,"%s\n", name);
   return 0;
}
...
static int f_error (const char *name, UNUSED_PAR2(CONFIG_VALUES *,dummy) /* unused */)
{
   fprintf(stderr,"%s\n", name);
   return -1;
}
...
static int f_show_config (const char *name, UNUSED_PAR2(CONFIG_VALUES *,val) /* unused */)
{
   CONFIG_BLOCK *block;
   CONFIG_ITEM *itemlist, *item;
   int item_no, i;
...
"---------------------------------------------------------------------------\n");
   if ( name[0] == '\0' || strcmp(name,"*") == 0 || strcmp(name,"all") == 0 ||
        strcmp(name,"no-internal") == 0 )
   {
      for ( block=(&first_config_block); block != (CONFIG_BLOCK *) NULL;
            block=block->next )
      {
         if ( strcmp(name,"no-internal") == 0 && strcmp(block->section,"_internal_") == 0 )
            continue;
         if ( *section )
         {
            if ( block->section == (char *) NULL )
      ...
      if ( item->internal.itype != 30 )
      {
         ...
         if ( item->size > 1 )
         {
            ...
            for (inum=1; inum<item->size; inum++)
            {
               ...
               case 32:
                  ...
                  break;
               case 35:
                  if ( data.bdata[inum] != data.bdata[0] )
                     all_shown = 0;
                  break;
               case 36:
                  if ( data.idata[inum] != data.idata[0] )
         ...
         for (inum=0; inum<item->size; inum++)
         {
            ...
            case 32:
               ...
               break;
            case 35:
               sprintf(message,"%u ",(unsigned int)data.bdata[inum]);
               break;
            case 36:
               sprintf(message,"%d ",data.idata[inum]);
               break;
...
static int t_add_to_text (const char *txt, char *buf, size_t lbuf, size_t *plcur)
{
   size_t lnew = strlen(txt);
   if ( (*plcur)+lnew+1 > lbuf )
      return -1;
   strcat(buf,txt);
   return 0;
}
 ...
 *
 *  The formatted current configuration (same as with 'SHOW' or 'LIST') is
 *  returned to user code in a user-provided buffer.
 *  If the buffer is too short, no or partial results may be returned.
 *
 *  @param item Pointer to selected configuration item structure
 *  @param buf  Buffer to be filled with formatted configuration content
 *  @param lbuf Size of provided buffer (including any trailing '\0' character).
 *
 *  @return Current configuration in text representation, as far as fitting into buffer.
*/
   ...
   *buf = '\0';
   ...
   if ( item->internal.itype > 0 && item->internal.itype != 50 )
   {
      if ( item->internal.itype != 30 )
      {
         ...
         if ( all_shown )
            t_add_to_text("all: ",buf,lbuf,&lfilled);
         for (inum=0; inum<item->size; inum++)
         {
            switch ( item->internal.itype )
            {
            ...
            t_add_to_text(message,buf,lbuf,&lfilled);
            if ( all_shown )
               break;
         }
      }
      else /* itype==30: plain text just needs to be copied */
         t_add_to_text((char *)item->data,buf,lbuf,&lfilled);
   }
   
   /* For function type we return an empty string since we do not store
      how the function got called, not even the last call.
      For all other types we return the formatted or plain text content. */
...
static int bool_config_val (const char *name, const char *text, 
   const char *lbound, const char *ubound, int strict, bool *bval)
{
   bool value, lower, upper;

   lower = 0; upper = 1;
   if ( !is_boolean(text) )
   {
      config_syntax_error(name,"Value is not a boolean");
      return -1;
   }
   value = read_boolean(text); /* Returns 0 or 1 even if text says "false" or "On" or "77" */
   if ( lbound != (char *) NULL )
   {
      if ( !is_boolean(lbound) )
      {
         config_syntax_error(name,"Lower bound is not a boolean");
         return -1;
      }
      lower = read_boolean(lbound); /* Returns 0 or 1 */
      if ( value < lower )
      {
fprintf(stderr,"%s: lower bound violation for %u >= %u\n", name, value, lower);
         if ( strict ) 
         {
            config_syntax_error(name,"Violates lower bound");
            return -1;
   ...
   if ( ubound != (char *) NULL )
   {
      if ( !is_boolean(ubound) )
      {
         config_syntax_error(name,"Upper bound is not a boolean");
         return -1;
      }
      upper = read_boolean(ubound); /* Returns 0 or 1 */
      if ( value > upper )
      {
fprintf(stderr,"%s: upper bound violation for %u (%s) <= %u (%s)\n", name, value, text, upper, ubound);
         if ( strict ) 
         {
            config_syntax_error(name,"Violates upper bound");
            return -1;
...
static bool read_boolean (const char *text)
{
   if ( strcmp(text,"1") == 0 ||
      ...
      return 1;
        ...
        abbrev(text,"False") )
      return 0;
   else if ( is_unsigned_number(text) ) /* Not satisfying is_boolean() though */
      return 1;
   else
      return 0;
}
...
char *read_config_lines()
{
...
#endif
#ifdef DEBUG_HCONFIG
   printf("^^^ read_config_lines: from stack: [%s]\n", line);
#endif
         return(line);
      }
   ...
   if ( cfg_file == (FILE *) NULL )
   {
      ...
      {
         if ( (cfg_file = fileopen(fn,READ_TEXT)) == (FILE *) NULL )
         {
            char message[1060];
            snprintf(message,sizeof(message),
            ...
            read_status = -2;
#ifdef DEBUG_HCONFIG
            printf("^^^ read_config_lines: null_return #1\n");
#endif
            goto null_return;
         }
         fileclose(cfg_file);
         ...
         else
         {
            ...
            snprintf(message,sizeof(message),
               "Preprocessor command is '%s'.",command);
            Information(message);
            if ( system(command) >= 0 )
               fn = tmp_fname;
            else
            {
               ...
               read_status = -3;
#ifdef DEBUG_HCONFIG
               printf("^^^ read_config_lines: null_return #2\n");
#endif
               goto null_return;
            }
         }
      ...
      if ( fn[0] == '\0' )
      {
         ...
         read_status = -1;
#ifdef DEBUG_HCONFIG
         printf("^^^ read_config_lines: null_return #3\n");
#endif
         goto null_return;
      }
      if ( (cfg_file = fileopen(fn,READ_TEXT)) == (FILE *) NULL )
      {
         ...
         read_status = -2;
#ifdef DEBUG_HCONFIG
         printf("^^^ read_config_lines: null_return #4\n");
#endif
         goto null_return;
      }
   }
   ...
   if ( do_read )
   {
      char *s = fgets(line,sizeof(line)-1,cfg_file);
#ifdef DEBUG_HCONFIG
      if ( s == NULL )
         printf("^^^ read_config_lines: fgets [NULL; %s]\n", line);
      else
         printf("^^^ read_config_lines: fgets [%s]\n", line);
      if ( feof(cfg_file) )
         printf("^^^ read_config_lines: feof!\n");
      if ( ferror(cfg_file) )
         printf("^^^ read_config_lines: ferror!\n");
#endif
      if ( s==NULL || feof(cfg_file) || ferror(cfg_file) )
      {
         if ( ferror(cfg_file) )
         {
         ...
         else if ( s!=NULL && feof(cfg_file) && line[0] != '\0' && line[0] != '\n' )
         {
            Warning("Bad line at end of configuration file (missing newline or such).");
            Warning("The following configuration text may or may not get ignored or cause an error:");
            Warning(line);
            read_status = -4;
         }
         fileclose(cfg_file);
         ...
         if ( *preprocessor != '\0' && *tmp_fname != '\0' )
            unlink(tmp_fname);
#ifdef DEBUG_HCONFIG
         printf("^^^ read_config_lines: null_return #5\n");
#endif
         goto null_return;
      }
      next_line = line;
   ...
   {
#ifdef DEBUG_HCONFIG
      printf("^^^ read_config_lines: next_line: [%s]\n", next_line);
#endif
...
#ifdef DEBUG_HCONFIG
   printf("^^^ read_config_lines: this_line: [%s]\n", this_line);
#endif
   return(this_line);
