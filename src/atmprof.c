...
Copyright (C) 1990, 1997, 1998, 2008, 2009, 2010, 2018, 2019  Konrad Bernloehr
 ...
 *    c) interpolation is linear in log(density) etc. rather than
 *       using cubic splines like available with the repolator code.
 *
 *  The profiles can be set up by atmosphere number <n> (which means searching
 *     for file atmprof<n>.dat in some known paths), or from a AtmProf
 *     structure (table part or CORSIKA 5-layer parameters as fall-back).
 *  The CORSIKA built-in profiles are expanded internally to tables first,
 *     with the relevant parameters only available from the AtmProf structure.
 *
 ...
 *  @short Initialize atmospheric profiles.
 *
 *  Atmospheric models are passed on from the data itself.
 *  For the interpolation of relevant parameters (density, thickness,
 *  index of refraction, ...) all parameters are transformed such
 *  that linear interpolation can be easily used.
 *
 *  @param aprof Pointer to an AtmProf structure (can be NULL to be
 *     replaced by the common one).
 *
 *  @return 0 (OK) or -1 (error, e.g. table available)
 *
*/
...
int init_atmprof_s (AtmProf *aprof)
{
   double alt,rho,thick,n_1;
   num_prof = 0;
   current_atmosphere = 0;

   if ( aprof == NULL )
      aprof = get_common_atmprof();

   if ( aprof->n_alt <= 1 && !aprof->have_lay5_param )
   {
      fflush(stdout);
      fprintf(stderr,"Atmosheric profile has neither useful table data nor\n"
         "known parameters for the CORSIKA 5-layer model.\n");
      return -1;
   }

   printf("Initialize simplified atmospheric profile interpolation from AtmProf structure\n"
          "with profile ID=%d (file: %s), 5-layer parameters %s.\n",
          aprof->atmprof_id, 
          (aprof->atmprof_fname!=NULL) ? aprof->atmprof_fname : "none or not known",
          aprof->have_lay5_param ? "known" : "not known");

   if ( aprof->n_alt > 1 ) /* Prefer table data, if we have that */
   {
      int ialt;
      num_prof = (aprof->n_alt <= MAX_PROFILE) ? aprof->n_alt : MAX_PROFILE;
      for (ialt=0; ialt<num_prof; ialt++)
      {
         alt = aprof->alt_km[ialt];
         rho = aprof->rho[ialt];
         thick = aprof->thick[ialt];
         n_1 = aprof->refidx_m1[ialt];
         p_alt[ialt] = alt*1e3; /* Altitude in file was in km */
         p_log_rho[ialt] = (rho>0.)?log(rho):-1000.;
         p_rho[ialt] = rho;
         p_log_thick[ialt] = (thick>0.)?log(thick):-1000.;
         p_log_n1[ialt] = (n_1>0.)?log(n_1):-1000.;
      }
      current_atmosphere = aprof->atmprof_id;
   }
   else if ( aprof->have_lay5_param )
   {
      double etadsn0 = 0.000283 * 994186.38 / 1222.656; /* CORSIKA default */;
      int ialt;
      num_prof = 6; /* 5 layers makes 6 supporting levels */
      for (ialt=0; ialt<6; ialt++)
      {
         alt = aprof->hlay[ialt]; /* Remains in [km] at this point */
         if ( ialt < 4 ) /* exponentially falling density */
         {
            rho = aprof->batm[ialt] * aprof->datm[ialt] * exp(-(alt*1e5)*aprof->datm[ialt]);
            thick = aprof->aatm[ialt] + aprof->batm[ialt] * exp(-(alt*1e5)*aprof->datm[ialt]);
         }
         else if ( ialt == 4 || ialt == 5 ) /* constant density */
         {
            rho = aprof->datm[4];
            thick = aprof->aatm[4] - (alt*1e5) * aprof->datm[4];
         }
         n_1 = etadsn0 * rho;
         p_alt[ialt] = alt*1e3; /* Altitude in file was in km */
         p_log_rho[ialt] = (rho>0.)?log(rho):-1000.;
      ...
      current_atmosphere = 100; /* Fake ID instead of zero */
   }
