...
Copyright (C) 1994, 1997, 2000, 2001, 2003, 2004, 2006, 2007, 2008, 2009, 2010, 2012, 2014, 2016, 2018, 2019  Konrad Bernloehr
...
int write_test_lrg (IO_BUFFER *iobuf)
{
   ...
   item_header.type = 9990;           /* test data */
   item_header.version = 1;          /* Version 1 */
   item_header.ident = 1;
   item_header.use_extension = 1;
   
   iobuf->msg_ext = 0;  /* Would be an ugly long list of messages */

   put_item_begin_with_flags(iobuf,&item_header,0,1);
   
   if ( sizeof(long) < 8 )
      ld = (1UL<<30)+1000;
   else
      ld = (1UL<<32)+1000;
   if ( ( data = (BYTE *) calloc(ld,1) ) == NULL )
   {
      Error("Allocation of large data block failed.\n");
      exit(1);
   }
   data[1] = 1; data[2] = 2; data[3] = 3;
   data[(1UL<<30)-1] = 4; data[(1UL<<30)] = 5; data[(1UL<<30)+1] = 6;
   data[ld-2] = 8; data[ld-1] = 9;

   /* Note: we cannot use put_vector_of_byte here because of int length counter */
   for (i=0; i<ld; i++)
      put_byte(data[i],iobuf);
...
int read_test_lrg (IO_BUFFER *iobuf)
{
   ...
   for (i=0; i<ld; i++)
   {
      BYTE k = get_byte(iobuf);
      if ( i==1 || i==2 || i==3 || i==(1UL<<30)-1 || i==(1UL<<30) || i==(1UL<<30)+1 || i==ld-2 || i==ld-1 )
      {
         if ( (i==1 && k!=1) || (i==2 && k!=2) || (i==3 && k!=3) ||
              (i==(1UL<<30)-1 && k!=4) || (i==(1UL<<30) && k!= 5) || (i==(1UL<<30)+1 && k!= 6) ||
              (i==ld-2 && k!=8) || (i==ld-1 && k!=9) )
         {
            Error("Data in large data block not as expected.\n");
            return -4;
         }
      }
      else if ( k != 0 )
...
void syntax(const char *prg)
{
   ...
   fprintf(stderr,"  -x  Include tests for large data blocks\n"
                  "      (filename extension .zst or .gz is recommended!).\n");
   exit(1);
}
...
# endif
   ...
   while ( argc > 1 && argv[1][0] == '-' )
   {
      ...
      {
         lrg_test = 1;
         Information("Including tests of large data blocks.");
         if ( sizeof(long) < 8 )
            iobuf->max_length = (1L<<31) - 1;
         else
            iobuf->max_length = (1L<<33) - 1;
      }
      argc--;
      argv++;
   }
   if ( argc > 1 )
   {
      if ( lrg_test )
      {
         if ( strstr(argv[1],".gz") == NULL && strstr(argv[1],".zst") == NULL &&
              strstr(argv[1],".bz2") == NULL && strstr(argv[1],".xz") == NULL &&
              strstr(argv[1],".lzo") == NULL && strstr(argv[1],".lzma") == NULL )
         {
            Warning("Large data block test will create BIG data files.\n");
         }
      }
   ...
   if ( lrg_test)
   {
      fprintf(stderr,"Write large data blocks.\n");
      write_test_lrg(iobuf);
   }
   ...
   if ( lrg_test )
   {
      fprintf(stderr,"Reading a large data block.\n");
      if ( find_io_block(iobuf,&item_header) < 0 )
      {
         Error("*** Finding I/O block 6 failed");
         exit(1);
      }
      if ( read_io_block(iobuf,&item_header) < 0 )
      {
         Error("*** Reading I/O block 6 failed");
         exit(1);
      }
      if ( read_test_lrg(iobuf) < 0 )
      {
         Error("*** Reading large data block failed");
         exit(1);
      }
   }
...
#endif
   if ( lrg_test )
      Information("Tests with large data blocks were successful.\n");
