...
Copyright (C) 2014, 2016, 2017, 2018, 2019, 2021, 2022, 2023  Konrad Bernloehr
...
static void syntax (char *program)
{
   printf("Rip out data for each telescope into individual files.\n\n");
   ...
   while ( argc > 1 || input_fname != NULL )
   {
    ...
    tmp_fname_tail[0] = '\0';
    {
       ...
       if ( (s = strrchr(input_fname,'/')) != NULL )
          strncpy(tmp_fname_head, s+1, sizeof(tmp_fname_head)-1);
       else
          strncpy(tmp_fname_head, input_fname, sizeof(tmp_fname_head)-1);
    ...
    for (;;) /* Loop over all data in the input file */
    {
      ...
      switch ( (int) item_header.type )
      {
         ...
         case IO_TYPE_SIMTEL_EVENT:
            ...
            {
               rc = write_simtel_centralevent(iobuf,&hsdata->event.central);
            }
                     ...
                     if ( (iobuf->output_file = f_teldata[itel]) != NULL )
                     {
                        if ( extract_televent == 1 )
                        {
                           ...
                           if ( hsdata->event.trackdata[itel].raw_known ||
                              ...
                              (void) write_simtel_trackevent(iobuf,&hsdata->event.trackdata[itel]);
                           rc = write_simtel_televent(iobuf,&hsdata->event.teldata[itel],-1);
                        }
                        else if ( extract_televent == 2 )
                        {
                           ...
                           if ( te->raw != NULL && te->raw->known )
                           {
                              if ( te->readout_mode > 0 ) /* Write only samples */
                                 rc = write_simtel_teladc_samples(iobuf,te->raw);
                              else /* or write only sums, never both */
                                 rc = write_simtel_teladc_sums(iobuf,te->raw);
                           }
                        }
                        else
                        {
