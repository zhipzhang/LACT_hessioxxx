/* ============================================================================
   ...
   Copyright (C) 1991, 1992, 1993, 2000, 2001, 2002, 2003, 2004, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2016, 2018, 2019, 2021, 2023, 2024  Konrad Bernloehr
...
/* -------------------- io_buffer_size_spec -------------------- */
/**
 *  @short Get I/O buffer size from a string with possible short-hand notation.
 *
 *  Common interpretation of length specification for an I/O buffer size
 *  or limits for the buffer size, given a string consisting of an
 *  integer or simple floating point number (including a decimal point)
 *  and an optional SI or IT style suffix. A 'k' or 'kb' suffix (case-
 *  insensitive) is interpreted as 1000 while a 'kib' suffix is 1024.
 *  In the same style 'm' or 'mb' (again case-insensitive) are 1000*1000
 *  while 'mib' is 1024*1024, and so on for g/gb/gib and t/tb/tib.
 *  Not defined for larger units than terabytes (and that only on systems
 *  where a 'long' is more than 4 bytes long).
 *
 *  Examples:
 *    "1000000", "100MIB", "700k", "1.5GiB", "2.0e9"
 *
 *  Not valid: "1.5e4MiB", "2e9", "10mega"
 *
 *  @param s Character string for buffers size.
 *
 *  @return Interpreted buffer size as a long integer.
 */

long io_buffer_size_spec(const char *s)
{
   if ( s == NULL )
      return 0;
   ...
   if ( sl > 1 )
   {
      if ( s[sl-1] == 'k' || s[sl-1] == 'K' )
         fact = 1000L;
      else if ( s[sl-1] == 'm' || s[sl-1] == 'M' )
         fact = 1000L*1000L;
      else if ( s[sl-1] == 'g' || s[sl-1] == 'G' )
         fact = 1000L*1000L*1000L;
      else if ( (s[sl-1] == 'g' || s[sl-1] == 'T') && sizeof(long) > 4 )
         fact = 1000L*1000L*1000L*1000L;
      else if ( sl > 2 )
      {
         if ( strcasecmp(s+sl-2,"kb") == 0 )
            fact = 1000L;
         else if ( strcasecmp(s+sl-2,"mb") == 0 )
            fact = 1000L*1000L;
         else if ( strcasecmp(s+sl-2,"gb") == 0 )
            fact = 1000L*1000L*1000L;
         else if ( strcasecmp(s+sl-2,"tb") == 0 && sizeof(long) > 4 )
            fact = 1000L*1000L*1000L*1000L;
      }
      else if ( sl > 3 )
      {
         if ( strcasecmp(s+sl-2,"kib") == 0 )
            fact = 1024L;
         else if ( strcasecmp(s+sl-2,"mib") == 0 )
            fact = 1024L*1024L;
         else if ( strcasecmp(s+sl-2,"gib") == 0 )
            fact = 1024L*1024L*1024L;
         else if ( strcasecmp(s+sl-2,"tib") == 0 && sizeof(long) > 4 )
            fact = 1024L*1024L*1024L*1024L;
      }
   }
   long mxe = 0;
...
IO_BUFFER *allocate_io_buffer (size_t buflen)
{
   ...
   /* If the size of the new buffer was not specified (buflen is zero),
      we first look for an environment variable "NEW_IO_BUFFER",
      and fall back to IO_BUFFER_INITIAL_LENGTH if buflen is still zero.
      Any buffer size smaller than IO_BUFFER_MINIMUM_SIZE
      will be upgraded to that. If there is an environment
      variable "MIN_IO_BUFFER", we also request an initial
      buffer size at least that long. */
   const char *snew = getenv("NEW_IO_BUFFER");
   if ( buflen <= 0 && snew != NULL )
      buflen = io_buffer_size_spec(snew);
   if ( buflen <= 0 )
      buflen = IO_BUFFER_INITIAL_LENGTH;
   else if ( buflen < IO_BUFFER_MINIMUM_SIZE )
      buflen = IO_BUFFER_MINIMUM_SIZE;
   const char *smin = getenv("MIN_IO_BUFFER");
   if ( smin != NULL )
   {
      long bl = io_buffer_size_spec(smin);
      if ( bl > 0 && (size_t) bl > buflen )
         buflen = bl;
   }
   ...
   /* Allow for an environment variable to increase the maximum buffer size,
      without repeating that part in every application. */
   const char *smax = getenv("MAX_IO_BUFFER");
   if ( smax != NULL )
   {
      long mxe = io_buffer_size_spec(smax);
      if ( mxe > buf->max_length )
         buf->max_length = mxe;
   }
   buf->aux_count = 0;
   buf->regular = 0;
...
# endif
#endif
   buf->msg_ext = 1;       /* By default report when a buffer gets extended. */

#ifdef EVENTIO_HAVE_TOTAL
   buf->total_input = buf->total_output = 0;
#endif
...
/* -------------------------- put_int16 ------------------------ */
/** @short Same as put_short(). */

void put_int16(int num, IO_BUFFER *iobuf)
{
   put_short(num, iobuf);
}
...
/**
 *  @short Put a vector of 2-byte integers on an I/O buffer.
 *         This is exactly like put_vector_of_short for most machines.
 */

void put_vector_of_int16 (const int16_t *vec, int num, IO_BUFFER *iobuf)
{
   int i;

   if ( vec == (const int16_t *) NULL )
   {
      for (i=0; i<num; i++)
         put_int16(0,iobuf);
      return;
...
/* --------------------------- get_int16 ---------------------- */
/** @short Same as get_short(). */

int get_int16(IO_BUFFER *iobuf)
{
   return get_short(iobuf);
}
...
void get_vector_of_int16 (int16_t *vec, int num, IO_BUFFER *iobuf)
{
   int i;
...
int next_subitem_type (IO_BUFFER *iobuf)
{
   ...
   if ( iobuf->r_remaining < 12 ) /* Not enough for any more headers */
      return -2;
...
int read_io_block (IO_BUFFER *iobuf, IO_ITEM_HEADER *item_header)
{
   int rc = 0;
   size_t length, rb = 0;
   int header_length = 16; /* Item header length without using extended length */
   if ( iobuf->item_extension[0] )
      header_length += 4;  /* With extended length the header is four bytes longer */
   ...
   if ( iobuf->item_length[0] > 0 )
   {
      if ( iobuf->buflen < iobuf->item_length[0]+header_length )
      {
         if ( extend_io_buffer(iobuf,0,
              iobuf->item_length[0]+header_length-iobuf->buflen) == -1 )
         {
      ...
      if ( iobuf->input_fileno >= 0 || iobuf->input_file != (FILE *) NULL )
      {
         ...
         if ( iobuf->input_fileno >= 0 )
         {
            rb = READ_BYTES(iobuf->input_fileno,
               (char *)(iobuf->buffer+header_length),length);
         }
         else if ( (rb = fread((void *)(iobuf->buffer+header_length),(size_t)1,(size_t)length,
              iobuf->input_file)) == 0 )
         {
            if ( ferror(iobuf->input_file) )
               rc = -1;
      ...
      else if ( iobuf->user_function != NULL )
      {
         ...
         rc = (iobuf->user_function)(iobuf->buffer+header_length,(long)length,3);
         if ( rc == 0 )
            rb = length;
         else
            rb = 0;
...
#ifdef EVENTIO_HAVE_TOTAL
   if ( rc == 0 )
      iobuf->total_input += iobuf->item_length[0]+header_length; /* Including header length */
#endif
...
int list_io_blocks (IO_BUFFER *iobuf, int verbosity)
{
   ...
   while ( (rc = find_io_block(iobuf,&item_header)) == 0 )
   {
      ...
      if ( verbosity >= 1 )
      {
         ...
         if ( verbosity >= 3 )
         {
            snprintf(msg,sizeof(msg)-1,", user_flag=%d, use_extension=%d",
               item_header.user_flag, item_header.use_extension);
            Output(msg);
         }
      }
      Output("\n");
