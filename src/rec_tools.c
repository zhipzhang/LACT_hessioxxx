...
Copyright (C) 2000, 2001, 2006, 2007, 2008, 2009, 2010, 2012, 2014, 2018, 2019  Konrad Bernloehr
   ...
   double *xoff, double *yoff)
{
   ...
   /* Rotate by zenith angle (90 deg. - altitude) around elevation (y) axis. */
   double cx = sin(altitude); /* cos(za) */
   double sx = cos(altitude); /* sin(za) */
 ...
 *  In the presence of imaging errors, an effective focal length
 *  should be used.
*/
   ...
   double *obj_azimuth, double *obj_altitude)
{
   ...
   else
   {
      ...
      /* Rotate (inverse) by zenith angle (90 deg. - altitude) around elevation (y) axis. */
      double cx = sin(altitude); /* cos(za) */
      double sx = cos(altitude); /* sin(za) */
