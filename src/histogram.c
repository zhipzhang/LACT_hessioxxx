/* ============================================================================
   ...
   Copyright (C) 1991, 2001, 2002, 2003, 2006, 2007, 2008, 2010, 2011, 2012, 2013, 2014, 2018, 2019, 2023  Konrad Bernloehr
...
#ifdef _REENTRANT
void histogram_lock (HISTOGRAM *histo /* unused unless _REENTRANT */)
{
   pthread_mutex_lock(&histo->mlock_this);
}
#else
void histogram_lock (UNUSED_PAR2(HISTOGRAM *,histo) /* unused unless _REENTRANT */)
{
}
#endif
...
#ifdef _REENTRANT
void histogram_unlock (HISTOGRAM *histo /* unused unless _REENTRANT */)
{
   pthread_mutex_lock(&histo->mlock_this);
}
#else
void histogram_unlock (UNUSED_PAR2(HISTOGRAM *,histo) /* unused unless _REENTRANT */)
{
}
#endif
...
_CLEAR_BUSY_(histo1);
   ...
   histo1->tentries += histo2->tentries;
...
/* --------------------- print_histogram_scaled -------------------- */
/**
 *  @short Print scaled contents of a histogram on the terminal
 *
 *  Showing the actual content of each bin.
 *  Only supported for types 'F' (float) and 'D' (double).
 *
 *  @param  histo  Pointer to histogram
 *          fact   Scaling factor or zero for normalized.
 *
 *  @return  (none)
 */
...
void print_histogram_scaled (HISTOGRAM *histo, double fact)
{
   ...
   {
      printf("Histogram %ld: Invalid histogram type '%c' for scaling.\n",
         histo->ident, histo->type);
      return;
   }
   if ( histo->tentries == 0 )
   ...
   if ( fact == 0. ) /* Normalized */
   {
      if ( (histo->extension)->content_all != 0. )
         fact = 1./(histo->extension)->content_all;
   }
...
_HLOCK_
   ...
   if ( ny > 0 )
   {
      ...
      printf("# Content: %12.7g total (%12.7g inside range)\n",
         (histo->extension)->content_all*fact, (histo->extension)->content_inside*fact);
      printf("\n");
      for ( iy = 0; iy < ny; iy++ )
      {
         printf("\n");
         for ( ix = 0; ix < nx; ix++ )
         {
            ...
            printf("   %g\t%g\t%g\n", xmin+dx*(ix+0.5), ymin+dy*(iy+0.5), z*fact);
         }
      }
   }
   else
