...
Copyright (C) 2015, 2016, 2017, 2018, 2021, 2022, 2023  Konrad Bernloehr
 ...
 *     1216 (atmospheric density profile) -  Only one needed (should be identical, duplicate)
 *   Once per telescope (and per run for raw & DST levels 0-2; just once for DST level 3):
 *       75 (metaparam)    -  Write after mapping of telescope ID (if mapped); global remains ID -1.
 ...
 *     2033 (MC pixel monitoring) - Write after mapping of telescope ID (if mapped)
 *   Per shower:
 *    once:
...
Options:
     ...
     --only-telescope: List of telescopes on command line rather than map file.
                       (Comma separated telescope IDs or ranges of telescope IDs.)
     ...
     --trigger n(...): Array trigger if the requested number of triggered telescopes
                       is found among the list of telescope IDs.
                       This option can be used multiple times.
                       Note that the number set by '--min-trg-tel' must still be matched.
...
static void syntax (const char *program)
{
   printf("A program for extracting data for a subset of simulated telescopes.\n\n");
   ...
   printf("                       (Comma separated telescope IDs or ranges of telescope IDs.)\n");
   ...
   printf("     --trigger n(...): Array trigger if the requested number of triggered telescopes\n");
   printf("                       is found among the list of telescope IDs.\n");
   printf("                       This option can be used multiple times.\n");
   printf("                       Note that the number set by '--min-trg-tel' must still be matched.\n");
...
/** A list of telescope IDs from which we can form an array trigger */

struct trigger_list
{
   int ntrg;      /**< Number of triggered telescopes needed for an array trigger */
   int ntel;      /**< Number of telescopes being part of this trigger list */
   int tel_in_trg[H_MAX_TEL+1];  /**< List of telescope IDs */
   struct trigger_list *next; /**< Next trigger list */
};

static struct trigger_list start_trigger_list; /** Beginning of all trigger lists */

/* ------------------- add_trigger -------------------- */
/**
 *  One or multiple array triggers can be defined. Add one such trigger.
 */

int add_trigger (const char *text);

int add_trigger (const char *text)
{
   int ntrg = atoi(text);
   if ( ntrg <= 0 || ntrg > H_MAX_TEL )
      return 0;
   char *s = strchr(text,'(');
   if ( s == NULL )
   {
   ...
   struct trigger_list *t = (struct trigger_list *) calloc(1,sizeof(struct trigger_list));
   if ( t == NULL )
      return -1;
   t->ntrg = ntrg;
   t->ntel = 0;
   ...
   for (i=0; i<=H_MAX_TEL; i++)
      t->tel_in_trg[i] = 0;
   while ( getword(s,&ipos,word,sizeof(word)-1,',','\n') > 0 )
   {
      int i1 = atoi(word), i2 = 0;
      if ( strchr(word,'-') != NULL )
         sscanf(word,"%d-%d",&i1,&i2);
      if ( i1 > 0 && i1 <= H_MAX_TEL && i2 == 0 )
      {
         t->tel_in_trg[i1] = 1;
      }
      else if ( i1 > 0 && i2 >= i1 && i2 <= H_MAX_TEL )
      {
         for ( i=i1; i<=i2; i++ )
            t->tel_in_trg[i] = 1;
      }
   }
   for (i=1; i<=H_MAX_TEL; i++)
   {
      if ( t->tel_in_trg[i] )
         t->ntel++;
   }
   struct trigger_list *ti = &start_trigger_list;
   while ( ti->next != NULL )
      ti = ti->next;
   if ( ti->ntrg == 0 || ti->ntel == 0 )
   {
      memcpy(ti,t,sizeof(struct trigger_list));
      free(t);
      t = ti;
   }
   else
   ...
   if ( t->ntel > 0 && t->ntrg > 0 )
      printf("Setting up an array trigger with >= %d telescope(s) out of a list of %d.\n",
         t->ntrg, t->ntel );
   return t->ntrg;
}

int got_a_trigger (CentralEvent *ce);
...
int got_a_trigger (CentralEvent *ce)
{
   ...
   while ( ti != NULL )
   {
      ...
      if ( ti->ntrg > 0 && ti->ntel > 0 )
      {
         ...
         {
            int tel_id = ce->teltrg_list[itel];
            if ( tel_id > 0 && tel_id <= H_MAX_TEL )
            {
               if ( ti->tel_in_trg[tel_id] )
                  k++;
            }
         }
         if ( k >= ti->ntrg )
...
int check_for_delayed_write(IO_ITEM_HEADER *item_header, UNUSED_PAR2(int, ifile), AllHessData *hsdata_out, IO_BUFFER *iobuf_out);
...
printf("Unexpected check_for_delayed_write(): type: %d, id: %ld)\n", type, id);
   ...
   if ( type != IO_TYPE_SIMTEL_EVENT && type != IO_TYPE_SIMTEL_MC_PE_SUM )
   {
...
#endif
         rc = write_simtel_mc_pe_sum(iobuf_out,&hsdata_out->mc_event.mc_pesum);
         ev_pe_sum = 0;
         if ( rc != 0 )
            return rc;
      }
...
printf("Delayed writing of data event %ld (current data block type: %d, id: %ld)\n", ev_hess_event, type, id);
         ...
         if ( ( hsdata_out->event.num_teldata >= min_trg ||
              ...
              ( start_trigger_list.ntrg <= 0 || got_a_trigger(&hsdata_out->event.central) ) )
         {
            set_tel_idx_ref(0);
            rc = write_simtel_event(iobuf_out,&hsdata_out->event,-1);
            ev_out++;
         }
         ev_hess_event = 0;
      }
   }
   else if ( type == IO_TYPE_SIMTEL_EVENT )
   {
...
printf("Delayed writing of data event %ld (current data block type: %d, id: %ld)\n", ev_hess_event, type, id);
   ...
   else if ( type == IO_TYPE_SIMTEL_MC_PE_SUM )
   {
...
printf("Now processing item type %d (ID=%ld) from file %d ...\n", 
  (int) item_header->type, item_header->ident, ifile);
   ...
   MetaParamList *mpl = (MetaParamList *) NULL;
   ...
   switch ( (int) item_header->type )
   {
      ...
      case IO_TYPE_SIMTEL_RUNHEADER: /* 2000 */
         ...
         if ( (rc = write_simtel_runheader(iobuf_out,&hsdata_out->run_header)) < 0 )
         {
...
printf("MC input configuration written\n");
      ...
      case IO_TYPE_MC_ATMPROF: /* 1216 */
         ...
         rc = write_io_block_to_file(iobuf, iobuf_out->output_file);
         if ( rc != 0 || verbose )
            printf("writing atmospheric density profile to output, rc=%d\n", rc);
#ifdef DEBUG_MERGE
printf("Atmospheric density profile written\n");
#endif
         break;

      /* =================================================== */
      case IO_TYPE_HISTORY: /* 70: How sim_hessarray was run and how it was configured. */
         /* Copy to output buffer without unpacking it, if possible ... */
         rc = write_io_block_to_file(iobuf, iobuf_out->output_file);
         if ( rc != 0 || verbose )
...
printf("History written\n");
      ...
      /* =================================================== */
      case IO_TYPE_METAPARAM: /* 75: Final value(s) of selected parameters */
         tel_id = item_header->ident; /* Telescope ID is in the header */
#ifdef DEBUG_MERGE
printf("Got metaparameters for ID = %d\n", tel_id);
#endif
         if ( tel_id < 0 ) /* Global */
            tel_id3 = tel_id;
         else
...
printf("ID = %d mapped to %d\n", tel_id, tel_id3);
#endif
         if ( tel_id >= 0 && tel_id3 < 0 )
            return 0;
         mpl = (MetaParamList *) calloc(1,sizeof(MetaParamList));
         rc = read_metaparam(iobuf, mpl);
         if ( rc != 0 || verbose )
         {
            printf("read_metaparam(), rc = %d\n",rc);
            if ( rc != 0 )
            {
               clear_metaparam(mpl);
               free(mpl);
         ...
         set_metaparam_id(mpl,tel_id3);
         rc = write_metaparam(iobuf_out, mpl);
         if ( rc != 0 || verbose )
            printf("writing meta-parameters to output, rc=%d\n", rc);
#ifdef DEBUG_MERGE
printf("Meta-parameters for telescope ID %d (from %d) written\n", tel_id3, tel_id);
#endif
         clear_metaparam(mpl);
         free(mpl);
         break;
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_CAMSETTINGS:  /* 2002: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_camsettings(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         ...
         rc = write_simtel_camsettings(iobuf_out, &hsdata_out->camera_set[itel3]);
#ifdef DEBUG_MERGE
printf("Camera settings written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_CAMORGAN:  /* 2003: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_camorgan(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         rc = write_simtel_camorgan(iobuf_out, &hsdata_out->camera_org[itel3]);
#ifdef DEBUG_MERGE
printf("Camera organization written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_PIXELSET:  /* 2004: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_pixelset(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         hsdata_out->pixel_set[itel3].tel_id = tel_id3;
         /* Write the modified camera organisation to output */
         rc = write_simtel_pixelset(iobuf_out, &hsdata_out->pixel_set[itel3]);
#ifdef DEBUG_MERGE
printf("Pixel setting written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_PIXELDISABLE:  /* 2005: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_pixeldis(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         hsdata_out->pixel_disabled[itel3].tel_id = tel_id3;
         /* Write the modified camera organisation to output */
         rc = write_simtel_pixeldis(iobuf_out, &hsdata_out->pixel_disabled[itel3]);
#ifdef DEBUG_MERGE
printf("Disabled pixel list written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_CAMSOFTSET:  /* 2006: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_camsoftset(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         hsdata_out->cam_soft_set[itel3].tel_id = tel_id3;
         /* Write the modified camera organisation to output */
         rc = write_simtel_camsoftset(iobuf_out, &hsdata_out->cam_soft_set[itel3]);
#ifdef DEBUG_MERGE
printf("Camera software settings written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_POINTINGCOR:  /* 2007: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_pointingco(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         hsdata_out->point_cor[itel3].tel_id = tel_id3;
         /* Write the modified camera organisation to output */
         rc = write_simtel_pointingcor(iobuf_out, &hsdata_out->point_cor[itel3]);
#ifdef DEBUG_MERGE
printf("Pointing corrections written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_TRACKSET:  /* 2008: Per-telescope block */
         tel_id = item_header->ident; /* Telescope ID is in the header */
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
...
printf("itel = %d, tel_id = %d;  itel3 = %d, tel_id3 = %d\n", itel, tel_id, itel3, tel_id3);
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_trackset(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         hsdata_out->tracking_set[itel3].tel_id = tel_id3;
         /* Write the modified camera organisation to output */
         rc = write_simtel_trackset(iobuf_out, &hsdata_out->tracking_set[itel3]);
#ifdef DEBUG_MERGE
printf("Tracking setup written for tel. ID %d -> %d\n", tel_id, tel_id3);
      ...
      }
      {
         ...
         rc = read_simtel_event(iobuf,evi,-1);
         if ( rc != 0 || verbose )
         {
...
exit(1);
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_MC_SHOWER: /* 2020: Expected to be identical */
         check_for_delayed_write(item_header, ifile, hsdata_out, iobuf_out);
         rc = read_simtel_mc_shower(iobuf,&hsdata->mc_shower);
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_mc_shower(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         mcshowers[ifile-1]++;
      ...
      case IO_TYPE_SIMTEL_MC_EVENT: /* 2021 */
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_mc_event(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         mcevents[ifile-1]++;
...
printf("Ignoring photons/photo-electrons data block\n");
      ...
      /* =================================================== */
      case IO_TYPE_SIMTEL_TEL_MONI: /* 2022: Per-telescope block */
         ...
         if ( rc != 0 || verbose )
         {
            printf("read_simtel_tel_monitor(), rc = %d\n",rc);
            if ( rc != 0 )
               return rc;
         }
         /* Use memcpy for copying simple struct without any pointers in it. */
         ...
         rc = write_simtel_tel_monitor(iobuf_out,&hsdata_out->tel_moni[itel3],
      ...
      case IO_TYPE_SIMTEL_MC_PIXMON:
         ...
         /* Use memcpy for copying simple struct without any pointers in it. */
         memcpy(&hsdata_out->mcpixmon[itel3],&hsdata->mcpixmon[itel],sizeof(hsdata->mcpixmon[0]));
         /* All we need to change is the telescope ID */
         hsdata_out->mcpixmon[itel3].tel_id = tel_id3;
         /* Write the modified laser calibration data for this telescope to output. */
         rc = write_simtel_mc_pixel_moni(iobuf_out,&hsdata_out->mcpixmon[itel3]);        
         break;

      /* =================================================== */
      case IO_TYPE_SIMTEL_LASCAL: /* 2023: Per-telescope block */
         tel_id = item_header->ident; // Telescope ID is in the header
         itel = find_in_tel_idx(tel_id,ifile);
         itel3 = find_out_tel_idx(tel_id,ifile);
         if ( itel3 >= 0 && itel3 < H_MAX_TEL )
         ...
         hsdata_out->tel_lascal[itel3].tel_id = tel_id3;
         /* Write the modified laser calibration data for this telescope to output. */
         rc = write_simtel_laser_calib(iobuf_out,&hsdata_out->tel_lascal[itel3]);        
         break;
      ...
      case IO_TYPE_SIMTEL_XTRGMASK: /* 2090 */
         if ( tms[ifile-1] == NULL )
...
int read_map(const char *map_fname)
{
   ...
   if ( map_fname == NULL )
   {
      fprintf(stderr,"Missing map filename.\n");
      exit(1);
   }
...
int main (int argc, char **argv)
{
   ...
   for (iarg=1; iarg<argc; iarg++)
   {
      if ( argv[iarg][0] == '-' && argv[iarg][1] != '\0' )
      {
         ...
         else if ( strcmp(argv[iarg],"--trigger") == 0 && iarg+1<argc )
         {
            int n=0;
            if ( (n = add_trigger(argv[iarg+1])) <= 0 )
            {
               fprintf(stderr,"Not a valid trigger: %s\n", argv[iarg+1]);
               exit(1);
            }
            /* Avoid surprising combination of trigger list and global minimum multiplicity.
               If you really want such a thing, use '--min-trg-tel' after all '--trigger'. */
            if ( n < min_trg )
               min_trg = n; 
            iarg++;
            continue;
                    ...
                    strcmp(argv[iarg],"--only-telescopes") == 0) && iarg+1<argc )
         {
            ...
            while ( getword(argv[iarg],&ipos,word,sizeof(word)-1,',','\n') > 0 )
            {
               ...
               if ( strchr(word,'-') != NULL )
               {
                  if ( sscanf(word,"%d-%d", &i1, &i2) == 2 )
                  {
                     if ( i1 > 0 && i2 > 0 && i2>=i1 )
                     {
                        for (i=i1; i<=i2; i++)
                           fprintf(tmp_file,"1 %d\n", i);
                     }
                  }                  
               }
               else
   ...
   printf("Requesting a minimum of %d triggered telescope(s) in any case.\n", min_trg);

   if ( input_fname1 == NULL )
   {
      fprintf(stderr,"Missing input filename. See '--help' for command-line syntax.\n");
      exit(1);
   }
   if ( output_fname == NULL )
   {
      fprintf(stderr,"Missing output filename. See '--help' for command-line syntax.\n");
      exit(1);
   }
   ...
   if ( iobuf1->max_length < 400000000L )
      iobuf1->max_length = 400000000L;
   if ( iobuf3->max_length < 400000000L )
...
printf("Processing loop: t1=%d, p1=%d, e1=%ld, d1=%d\n",
   this_type1, prev_type1, event1, done1);
      ...
      if ( done1 )
      {
         if ( done1 )
         {
            printf("Reading from input file completed (%ld events in, %ld out).\n",
               ev_in, ev_out);
            if ( this_type1 == 100 )
            {
...
printf("Type1 = %d, ID = %ld\n", this_type1, item_header1.ident);
         ...
         switch ( this_type1 )
         {
            ...
            case 2000: /* IO_TYPE_SIMTEL_RUNHEADER: Merging without immediate output (waits for file 2) */
            case 2001: /* IO_TYPE_SIMTEL_MCRUNHEADER: Merging without immediate output (waits for file 2) */
            ...
            case 2002: /* IO_TYPE_SIMTEL_CAMSETTINGS */
            case 2003: /* IO_TYPE_SIMTEL_CAMORGAN */
            case 2004: /* IO_TYPE_SIMTEL_PIXELSET */
            case 2005: /* IO_TYPE_SIMTEL_PIXELDISABLE */
            case 2006: /* IO_TYPE_SIMTEL_CAMSOFTSET */
            case 2007: /* IO_TYPE_SIMTEL_POINTINGCOR */
            case 2008: /* IO_TYPE_SIMTEL_TRACKSET */
            case 2022: /* IO_TYPE_SIMTEL_TEL_MONI */
            case 2023: /* IO_TYPE_SIMTEL_LASCAL */
               if ( this_type1 < 2020 )
                  event1 = -1;
               break;

            case 2020: /* IO_TYPE_SIMTEL_MC_SHOWER */
            case 2021: /* IO_TYPE_SIMTEL_MC_EVENT */
            case 2026: /* IO_TYPE_SIMTEL_MC_PE_SUM */
            case 1204: /* IO_TYPE_MC_TELARRAY */
            case 2010: /* IO_TYPE_SIMTEL_EVENT */
               if ( this_type1 == 2020 )
