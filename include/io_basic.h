/* ============================================================================
   ...
   Copyright (C) 1991, 2001, 2007, 2009, 2010, 2014, 2016, 2018, 2019, 2021, 2023, 2024  Konrad Bernloehr
...
/* Indicate add-on features */
#define EVENTIO_HAVE_TOTAL 1
...
struct _struct_IO_BUFFER
{
   ...
   */
   ...
   int msg_ext;       /**< 1 if buffer extension should be reported, 0 if not */
#ifdef EVENTIO_HAVE_TOTAL
   long total_input;  /**< Sum of bytes read by read_io_block calls */
   long total_output; /**< Sum of bytes written by write_io_block calls */
#endif
};
...
void put_int16 (int num, IO_BUFFER *iobuf); /* Same as put_short */
int get_short (IO_BUFFER *iobuf);
int get_int16 (IO_BUFFER *iobuf);           /* Same as get_short */
/* Internally 'int' (typically 32 bits) but stored as 'short' (16 bits) */
