...
/**
 *  A more complete, alternative bunch structure which can also
 *  represent upward-going photon bunches or horizontal ones
 *  while the bunch and cbunch structures strictly assume
 *  downward-going photon bunches. While the coordinates of the
 *  sphere is still needed as extra information, for example
 *  its height for evaluating atmospheric extinction, the
 *  height of emission was replaced by the distance between
 *  emission and arrival position - no matter where this
 *  supposed arrival position is w.r.t. the fiducial sphere.
 */

struct bunch3d
{
   float photons; /**< Number of photons in bunch */
   float x, y, z; /**< Arrival position relative to telescope (cm), */
                  /**< with the fiducial sphere center at (0,0,0). */
   float cx, cy, cz;  /**< Direction cosines of photon direction, */
                  /** usually with cz < 0 for downward. */
   float ctime;   /**< Arrival time (ns) */
   float dist;    /**< Distance of emission point from arrival position (cm) */
   float lambda;  /**< Wavelength in nanometers or 0 */
};
